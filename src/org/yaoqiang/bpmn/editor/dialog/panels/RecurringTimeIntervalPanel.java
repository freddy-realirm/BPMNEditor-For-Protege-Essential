...
public class RecurringTimeIntervalPanel extends XMLPanel {
	...
	protected SpinnerPanel cyclePanel = new SpinnerPanel(0, 0, Integer.MAX_VALUE, 45);
	...
	public RecurringTimeIntervalPanel(final BPMNPanelContainer pc, XMLElement owner) {
		this(pc, owner, true);
	}

	public RecurringTimeIntervalPanel(final BPMNPanelContainer pc, XMLElement owner, boolean enabled) {
		super(pc, owner);
		...
		cyclePanel.getSpinner().addChangeListener(changeListener);
		...
		JLabel cl = new JLabel(Resources.get("cycle") + ": ");
		...
		JButton startButton = new JButton(new ImageIcon(RecurringTimeIntervalPanel.class.getResource("/org/yaoqiang/bpmn/editor/images/calendar.gif")));
		...
		JButton endButton = new JButton(new ImageIcon(RecurringTimeIntervalPanel.class.getResource("/org/yaoqiang/bpmn/editor/images/calendar.gif")));
		...
		startEndPanel.add(cl);
		startEndPanel.add(Box.createHorizontalStrut(5));
		startEndPanel.add(cyclePanel);
		...
		startEndPanel.add(Box.createHorizontalStrut(14));
	...
	public String buildExpression() {
		...
		if (expression.length() == 0) {
			if (start.length() == 0 || end.length() == 0) {
				JOptionPane.showMessageDialog(null, "Invalid recurring time interval!", Resources.get("Warning"), JOptionPane.WARNING_MESSAGE);
				return "";
			} else {
		...
		String cycle = cyclePanel.getSpinner().getModel().getValue().toString();
		if (!cycle.equals("0")) {
			expression = "R" + cycle + "/" + expression;
		} else {
